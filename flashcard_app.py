...
class FlashcardApp:
    ...
    def create_main_menu(self):
        ...
        tk.Label(self.root, text="What would you like to do?", font=("Helvetica", 24, "bold"), fg='pink', bg='white').pack(pady=20)  # Main menu label
        tk.Button(self.root, text="learn", command=self.start_learning, bg='#ff66b2', fg='black', font=("Helvetica", 20), height=2, width=15).pack(pady=10)  # Learn button
        tk.Button(self.root, text="add", command=self.add_flashcards, bg='#ffb380', fg='black', font=("Helvetica", 20), height=2, width=15).pack(pady=10)  # Add flashcards button
        tk.Button(self.root, text="quit", command=self.root.quit, bg='#666666', fg='white', font=("Helvetica", 20), height=2, width=15).pack(pady=10)  # Quit button
    ...
    def choose_topic(self, action):
        ...
        tk.Label(self.root, text="Choose a topic", font=("Helvetica", 20), bg='white').pack(pady=20)  # Topic selection label
        tk.Label(self.root, text="Click on the dropdown menu to see the list of topics.", font=("Helvetica", 14), bg='white').pack(pady=5)  # Instruction label
        ...
        if action == "add":
            topic_options = self.topics + ["Create new topic"] if self.topics else ["Create new topic"]  # Add 'Create new topic' option if adding flashcards
        else:
        ...
        tk.Button(self.root, text="Proceed", command=proceed, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=10)  # Proceed button
        tk.Button(self.root, text="Back", command=self.create_main_menu, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=5)  # Back button
    ...
    def create_new_topic(self, action):
        ...
        tk.Label(self.root, text="Enter the name of the new topic", font=("Helvetica", 20), bg='white').pack(pady=20)  # New topic label
        new_topic_entry = tk.Entry(self.root, width=50, font=("Helvetica", 14))  # Entry widget for entering new topic name
        ...
        def save_new_topic():
            ...
            if new_topic:
                if not self.deck.new_topic_dictionary(new_topic):  # Check if the topic already exists
                    ...
                    messagebox.showinfo("Info", f"Topic '{new_topic}' created successfully!")  # Display info message for successful creation
        ...
        tk.Button(self.root, text="Save Topic", command=save_new_topic, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=10)  # Save topic button
        tk.Button(self.root, text="Back", command=lambda: self.choose_topic(action), bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=5)  # Back button
    ...
    def confirm_reset_or_start(self):
        ...
        tk.Label(self.root, text=f"Topic: {self.current_topic}", font=("Helvetica", 20), bg='white').pack(pady=20)  # Display current topic
        tk.Button(self.root, text="Start Learning", command=self.check_topic_completion, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=5)  # Start learning button
        tk.Button(self.root, text="Reset Progress", command=self.reset_progress, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=5)  # Reset progress button
        tk.Button(self.root, text="Back", command=self.create_main_menu, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=5)  # Back button
    ...
    def reset_progress(self):
        ...
        messagebox.showinfo("Info", f"Progress for topic '{self.current_topic}' has been reset.")  # Display info message
        self.confirm_reset_or_start()  # Proceed to confirm reset or start learning
    ...
    def check_topic_completion(self):
        ...
        """
        self.learn_topic = LearnTopic(self.deck.get_topic_dictionary(self.current_topic), self.deck)  # Create LearnTopic instance for the current topic
        if not self.learn_topic.still_has_flashcards():  # If no flashcards left to learn
            messagebox.showinfo("Info", f"You have completed the topic '{self.current_topic}'. Please reset the progress to start learning again.")  # Display info message
            self.confirm_reset_or_start()  # Proceed to confirm reset or start learning
        else:
    ...
    def show_learning_instructions(self):
        ...
        tk.Label(self.root, text=instructions, font=("Helvetica", 16), bg='white', wraplength=400).pack(pady=20)  # Display learning instructions
        tk.Button(self.root, text="Start Learning", command=self.start_learning_topic, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=10)  # Start learning button
        tk.Button(self.root, text="Back", command=self.confirm_reset_or_start, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=5)  # Back button
    ...
    def show_flashcard(self):
        ...
        tk.Label(self.root, text=f"Your flashcard is: {self.current_question}", font=("Helvetica", 20), bg='white').pack(pady=20)  # Display the flashcard question
        ...
        def show_answer():
            ...
            tk.Label(self.root, text=f"The answer is: {self.current_answer}", font=("Helvetica", 20), bg='white').pack(pady=20)  # Display the answer
            tk.Button(answer_frame, text="I knew this", command=lambda: self.update_and_show_result(True), bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(side=tk.LEFT, padx=10)  # Button for 'I knew this'
            tk.Button(answer_frame, text="I did not know this", command=lambda: self.update_and_show_result(False), bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(side=tk.LEFT, padx=10)  # Button for 'I did not know this'
            tk.Button(answer_frame, text="Exit", command=lambda: self.show_progress(topic_completed=False), bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(side=tk.LEFT, padx=10)  # Exit button

        tk.Button(answer_frame, text="Show Answer", command=show_answer, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=10)  # Show answer button
    ...
    def update_and_show_result(self, knew_answer):
        ...
        """
        self.learn_topic.update_card_progress(self.current_question, knew_answer)  # Update progress based on user response
    ...
    def show_result(self, knew_answer):
        ...
        tk.Button(self.root, text="Next Flashcard", command=self.show_flashcard, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=10)  # Next flashcard button
        tk.Button(self.root, text="Exit", command=lambda: self.show_progress(topic_completed=False), bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=5)  # Exit button
    ...
    def show_progress(self, topic_completed=False):
        ...
        if topic_completed:
            tk.Label(self.root, text="Congratulations! You have completed the topic.", font=("Helvetica", 20), bg='white').pack(pady=20)  # Topic completed message

        tk.Label(self.root, text="Learning Session Complete", font=("Helvetica", 20), bg='white').pack(pady=20)  # Session complete message
        tk.Label(self.root, text=f"Correct answers: {self.learn_topic.correct_answers}", font=("Helvetica", 18), bg='white').pack(pady=5)  # Display correct answers
        tk.Label(self.root, text=f"Wrong answers: {self.learn_topic.wrong_answers}", font=("Helvetica", 18), bg='white').pack(pady=5)  # Display wrong answers
        tk.Label(self.root, text=f"Finished flashcards: {self.learn_topic.finished_cards}", font=("Helvetica", 18), bg='white').pack(pady=5)  # Display finished flashcards

        tk.Button(self.root, text="Back to Main Menu", command=self.create_main_menu, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=20)  # Back to main menu button
    ...
    def add_flashcard_to_topic(self):
        ...
        tk.Label(self.root, text=f"Adding flashcards to {self.current_topic}", font=("Helvetica", 20), bg='white').pack(pady=20)  # Label indicating topic for adding flashcards
        ...
        card_front.insert(0, "Enter the front of the flashcard")  # Default text for flashcard front
        ...
        card_back.insert(0, "Enter the back of the flashcard")  # Default text for flashcard back
        ...
        def add_card():
            ...
            if front and back:  # If both front and back are non-empty
                self.deck.add_new_flashcard(self.current_topic, front, back)  # Add the new flashcard to the selected topic
                self.deck.save_deck_json()  # Save the updated deck to JSON file
        ...
        tk.Button(self.root, text="Add Flashcard", command=add_card, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=10)  # Add flashcard button
        tk.Button(self.root, text="Back to Main Menu", command=self.create_main_menu, bg='#ff66b2', fg='black', font=("Helvetica", 18)).pack(pady=5)  # Back to main menu button
